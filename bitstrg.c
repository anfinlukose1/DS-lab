...
#include <stdlib.h>
#define clrscr();
// type dif struct{unsigned car x:1;}bit;
...
void main()
{
    ...
    void differnce(struct bit[], struct bit[], struct bit[]);

    struct bit a[10] = {0}, b[10] = {0}, c[10] = {0};
    int n;

    printf("number of elements in a set A :");
    scanf("%d", &n);  
    readset(a, n);

    printf("number of elements in a set B :");
    scanf("%d", &n);
    readset(b, n);
...
void differnce(struct bit a[], struct bit b[], struct bit c[])
{
    int k;
    for(k = 0; k < size; ++k)
        c[k].x = a[k].x & (~b[k].x); 
